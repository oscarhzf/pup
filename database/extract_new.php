...
function extractTemp($conn, $id, $date){
    $sql = "SELECT date, time, tempC From engtemp WHERE id = '$id' AND date = '$date' ORDER BY date, time";
...
function extractSpeed($conn, $id, $date){
    $sql = "SELECT date, time, speed From data WHERE id = '$id' AND date = '$date' ORDER BY date, time";
    $result = mysqli_query($conn, $sql);
    if(mysqli_num_rows($result)>0){
        $content = array();
        while($row = mysqli_fetch_assoc($result)){
...
function extractGPS($conn, $id, $date){
    $sql = "SELECT date, time, latitude, longitude From data WHERE id = '$id' AND date = '$date' ORDER BY date, time";
    $result = mysqli_query($conn, $sql);
    if(mysqli_num_rows($result)>0){
        $content = array();
        while($row = mysqli_fetch_assoc($result)){
